...
/* rk_ui.c(UI_RKADK_ENGINE) 提供的 UI 层送显接口:
 * 把整幅 OSD 画布(ARGB8888, 透明=不遮挡)与当前 UI 帧合成后送显。
 * 显示链路是 RKADK_UI(SPLICE_MODE_RGA) 拼接上屏, RGN 挂 VPSS 在该链路上
 * 不生效(画面无框), 因此检测结果改走 UI 合成; 未编入 rk_ui 时弱符号返回 -1,
 * 仍保留 RGN 位图刷新作为备用通道。 */
extern int rk_ui_osd_commit(const void *canvas, uint32_t width, uint32_t height);
__attribute__((weak)) int rk_ui_osd_commit(const void *canvas, uint32_t width,
										   uint32_t height) {
	(void)canvas;
	(void)width;
	(void)height;
...
static void disp_osd_show(const det_box_t *boxes, int n) {
	...
	/* 主通道: 经 RKADK_UI 把 OSD 画布与 UI 帧合成送显(RGN 在此显示链路不生效) */
	rk_ui_osd_commit(g_disp_osd_canvas, DISP_OSD_WIDTH, DISP_OSD_HEIGHT);
}
