 ...
 * 每次推理完成后把检测框 + 类别 index 重绘到位图并 RK_MPI_RGN_SetBitMap 刷新。
...
#define DISP_OSD_FONT_SCALE      2      /* index 数字放大倍数(5x7 字体) */
...
/* 在 (x,y) 处绘制类别 index(带半透明黑色背景) */
static void osd_draw_index(int x, int y, int index, uint32_t color) {
	char buf[12];
	...
	cx = x + pad;
	for (int i = 0; i < len; i++) {
		const uint8_t *glyph = g_osd_font_5x7[buf[i] - '0'];
...
static void disp_osd_get_chn(MMF_CHN_S *stChn) {
...
static int disp_osd_attach(void) {
	...
	stRegion.unAttr.stOverlay.enPixelFormat = RK_FMT_ARGB8888;
	...
	stChnAttr.unChnAttr.stOverlay.enPixFormat = RK_FMT_ARGB8888;
	stChnAttr.unChnAttr.stOverlay.stPoint.s32X = 0;
	stChnAttr.unChnAttr.stOverlay.stPoint.s32Y = 0;
	stChnAttr.unChnAttr.stOverlay.u32Layer = 0;
...
static void disp_osd_show(const det_box_t *boxes, int n) {
	...
	for (int i = 0; i < n; i++) {
		...
		osd_draw_index(x1 + DISP_OSD_LINE_THICK, y1 + DISP_OSD_LINE_THICK,
					   boxes[i].cls, color);
	}
			...
			if (ret == RKNN_SUCC && rknn_query(g_rknn_detect_ctx,
				...
				if (rknn_outputs_get(g_rknn_detect_ctx, io_num.n_output,
					...
					for (int i = 0; i < box_cnt; i++) {
						printf("det[%d/%d]: cls=%d conf=%.2f box=(%.0f,%.0f)-(%.0f,%.0f)\n",
							   i + 1, box_cnt, g_det_boxes[i].cls, g_det_boxes[i].conf,
							   g_det_boxes[i].x1, g_det_boxes[i].y1,
							   g_det_boxes[i].x2, g_det_boxes[i].y2);
					}
					/* 把检测框 + 类别 index 叠加到 DISP 流 */
